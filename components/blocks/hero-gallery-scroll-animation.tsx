...
const ContainerScale = React.forwardRef<HTMLDivElement, HTMLMotionProps<"div">>(
  ({ className, style, ...props }, ref) => {
    ...
    const pointerEvents = useTransform(scrollYProgress, (pos) => pos > 0.35 ? "none" : "auto")
    const display = useTransform(scrollYProgress, (pos) => pos > 0.45 ? "none" : "block")
    ...
    return (
      <motion.div
        ref={ref}
        className={cn("left-1/2 top-1/2 size-fit z-50", className)}
        style={{
          x: "-50%",
          y: "-50%",
          scale,
